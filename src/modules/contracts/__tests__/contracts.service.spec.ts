...
describe('ContractsService', () => {
    ...
    beforeEach(async () => {
        ...
        contractModel = moduleRef.get<Model<Contract>>(
            getModelToken(Contract.name, 'qng_mainnet')
        )
    })
    ...
    describe('create', () => {
        it('should create contract', async () => {
            ...
            jest.spyOn(contractModel, 'create').mockImplementation(() =>
                Promise.resolve({
                    ...mockContract1,
                } as any)
            )
        ...
        it('should throw an error if createdHash does not match the contract address', async () => {
            ...
            await expect(
                contractsService.create(createContractDto)
    ...
    describe('findAllDetail', () => {
        it('should return an array of contracts', async () => {
            ...
            const selectField =
                'address createdBlock lastScannedBlock scannable scanning abi'
            const filteredContracts = mockContracts.filter(
                contract =>
                    contract.scannable === queryCriteria.scannable &&
                    contract.scanning === queryCriteria.scanning
            )
            ...
            await expect(
                contractsService.findAllDetail(queryCriteria, selectField)
            ).resolves.toEqual(finalContracts)

            expect(contractModel.find).toHaveBeenCalledWith(queryCriteria)
            expect(contractModel.find().select).toHaveBeenCalledWith(
                selectField
            )
        })
    })
    ...
    describe('delete', () => {
        it('should delete a contract by address', async () => {
            ...
            expect(
                contractModel.findOneAndDelete().select
            ).toHaveBeenCalledWith('+abi')
        })
    ...
    describe('update', () => {
        it('should update a contract by address', async () => {
            ...
            expect(
                await contractsService.update(address, updateContractDto)
            ...
            expect(
                contractModel.findOneAndUpdate().select
            ).toHaveBeenCalledWith('+abi')
        })
        ...
        it("should throw an error if contract doesn't exist", async () => {
            ...
            await expect(
                contractsService.update(address, updateContractDto)
            ).rejects.toThrow(HttpException)
        })
    })
    ...
    describe('clearEvents', () => {
        it('should stop scanning a contract', async () => {
            ...
            const expectedResult = {
                ...mockContract1,
                lastScannedBlock: mockContract1.createdBlock
                    ? mockContract1.createdBlock
                    : 0,
            }
            jest.spyOn(contractsService, 'findOne').mockResolvedValue(
                mockContractWithoutAbi
