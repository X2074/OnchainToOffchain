...
export class EventsService {
    ...
    async getEventsStatistics(
        ...
        let selectField = Object.keys(options).join(' ') + ' time'
        ...
        times.forEach(time => {
            statisticsData[time] = this.buildMultiLevelObject(
                [...groups, 'options'],
                {
                    ...groupFields,
        ...
        times.forEach(time => {
            statisticsData[time] = this.deepStatistics(
                statisticsData[time],
                options
            )
        })
        ...
        const result = []
        times.forEach(time => {
            result.push({ time, ...statisticsData[time] })
        })
        return result
    }
        ...
        Object.keys(obj).forEach(key => {
            if (Array.isArray(obj[key])) {
                switch (options[key]) {
                    ...
                    case 'average':
                        obj[key] = (
                            obj[key].reduce((accumulator, current) => {
                                return accumulator + BigInt(current)
                            }, 0n) / BigInt(obj[key].length)
                        ).toString()
                        break
                    case 'min':
                        obj[key] = obj[key]
